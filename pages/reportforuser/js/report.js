Vue.createApp({
  ...
  mounted() {
    ...
    this.url_base_app =
      window.location.protocol + "//" + window.location.host + "/adminphp/";
    // this.get_ven_all()
    this.get_ven_coms();
  },
  watch: {},
  methods: {
    get_ven_coms() {
      // this.isLoading = true
      axios
        .post("../../server/asu/report/get_ven_coms.php")
        .then((response) => {
    ...
    get_ven_all() {
      // this.isLoading = true
      axios
        .get("../../server/asu/report/reportK.php")
        .then((response) => {
    ...
    print(vcid) {
      // this.isLoading = true
      axios
        .post("../../server/asu/report/report.php", { vcid: vcid })
        .then((response) => {
          if (response.data.status) {
    ...
    print2(vcid) {
      // this.isLoading = true
      axios
        .post("../../server/asu/report/report2.php", { vcid: vcid })
        .then((response) => {
          if (response.data.status) {
            var print = JSON.stringify(response.data);
            localStorage.setItem("print", print);
    ...
    print3(ven_month) {
      axios
        .post("../../server/asu/report/report3.php", { ven_month: ven_month })
        .then((response) => {
          if (response.data.status) {
            var print = JSON.stringify(response.data);
            localStorage.setItem("print", print);
    ...
    print4(ven_month, ven_com_num, ven_com_date) {
      axios
        .post("../../server/asu/report/report4.php", {
          ven_month: ven_month,
    ...
    con_f(ven_month) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.isLoading = true;
          axios
            .post("../../server/asu/report/conf.php", { ven_month: ven_month })
            .then((response) => {
              if (response.data.status) {
                this.alert("success", response.data.message, 1000);
    ...
    uptogcal(ven_com_idb) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.isLoading = true;
          axios
            .post("../../server/asu/report/send_to_gcal.php", {
              ven_com_id: ven_com_idb,
            })
            .then((response) => {
              if (response.data.status) {
                this.alert("success", response.data.message, 1000);
