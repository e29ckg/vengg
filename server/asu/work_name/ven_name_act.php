...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try{
        if($act == 'insert'){
            ...
            isset($ven_name->name_full)  ?  $name_full = $ven_name->name_full      : array_push($errors,'ชื่อเต็ม'); 
            ...
            $sql = "INSERT INTO ven_name(name, name_full, DN, srt) VALUE(:name, :name_full, :DN, :srt);";        
            $query = $conn->prepare($sql);
            $query->bindParam(':name',$name, PDO::PARAM_STR);
            $query->bindParam(':name_full',$name_full, PDO::PARAM_STR);
        ...
        if($act == 'update'){
            $id     = $ven_name->id;
            $name   = $ven_name->name;
            $name_full   = $ven_name->name_full;
            $DN     = $ven_name->DN;
            $srt    = (int)$ven_name->srt;

            $sql = "UPDATE ven_name SET name =:name, name_full=:name_full, DN=:DN, srt=:srt WHERE id = :id";   
