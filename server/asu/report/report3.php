...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try{
        ...
        foreach($res_ven_com as $rvc){
            if($rvc->ven_name == 'ฟื้นฟู/ตรวจสอบการจับ'){
                $ven_com .= $rvc->ven_com_num; 
                $ven_com .= '(กลางวัน)'; 
                $ven_com .= ', '; 
            }
            if($rvc->ven_name == 'หมายจับ-ค้น'){
                $ven_com .= $rvc->ven_com_num; 
                $ven_com .= '(กลางคืน)'; 
            }
            $ven_com .= ' '; 
        ...
        if($query->rowCount() > 0){                        //count($result)  for odbc
           ...
           foreach($ven_date_arr as $date){
                ...
                foreach($u_name_arr as $u){
                    if($date == $u['ven_date']){
                        if($u['u_role'] == 'ผู้พิพากษา' && $u['ven_name'] == 'ฟื้นฟู/ตรวจสอบการจับ'){
                            $u1 = $u['u_name'];
                        }
                        if($u['u_role'] == 'ผู้พิพากษา' && $u['ven_name'] == 'หมายจับ-ค้น'){
                            $u2 = $u['u_name'];
                        }
                        if($u['u_role'] == 'ผอ./แทน' && $u['ven_name'] == 'ฟื้นฟู/ตรวจสอบการจับ'){
                            $u3 = $u['u_name'];
                        }
                        if($u['u_role'] == 'จนท' && $u['ven_name'] == 'ฟื้นฟู/ตรวจสอบการจับ'){
                            array_push($u45,$u['u_name']);
                        }
                        
                        if($u['u_role'] == 'จนท' && $u['ven_name'] == 'หมายจับ-ค้น'){
                            $u6 = $u['u_name'];
                        }
                        if($u['u_role'] == 'ผู้ตรวจ' && $u['ven_name'] == 'ผู้ตรวจ(กลางคืน)'){
                            $u7 = $u['u_name'];
                        }
                    }                    
                }
